	...
	addProfile() {
	  ...
	  if (!this.profileData.valid) {
		...
		firebase.database().ref('/profile/'+this.authService.getUserID()).update({
			   ...
			   starttime : (this.profileData.value.starttime).substring(((this.profileData.value.starttime).indexOf("T") + 1),(this.profileData.value.starttime).lastIndexOf(".")),
			   endtime : (this.profileData.value.endtime).substring(((this.profileData.value.endtime).indexOf("T") + 1),(this.profileData.value.endtime).lastIndexOf(".")),
